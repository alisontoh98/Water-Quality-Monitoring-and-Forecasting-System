...
query = "SELECT * FROM water_quality_data WHERE FISH_TANK_NUMBER = 2"
...
lstm_model.save('Model/model2_do.h5')
keras.models.load_model('Model/model2_do.h5')
...
lstm_model.save('Model/model2_ph.h5')
keras.models.load_model('Model/model2_ph.h5')
train_dates = pd.to_datetime(df['DATE_TIME'])
n_past = 1
n_days_for_prediction= 8
...
lstm_model.save('Model/model2_temp.h5')
keras.models.load_model('Model/model2_temp.h5')
train_dates = pd.to_datetime(df['DATE_TIME'])
n_past = 1
n_days_for_prediction= 8
...
df_forecast = pd.DataFrame({'FISH_TANK_NUMBER':'2','FORCA_DATE':np.array(forecast_dates), 'FORCA_DISSOLVED_OXYGEN':y_pred_future_do, 'FORCA_PH_VALUE':y_pred_future_ph, 'FORCA_WATER_TEMPERATURE':y_pred_future_temp})
